...
from model_def import resnet3D_utils as resnet_utils
...
def bottleneck(inputs,
  ...
  with variable_scope.variable_scope(scope, 'bottleneck_v2', [inputs]) as sc:
    ...
    residual = layers_lib.conv2d(
        preact, depth_bottleneck, [1, 1, 1], stride=1, scope='conv1')
  ...
  with variable_scope.variable_scope(
    ...
    with arg_scope(
      ...
      if include_root_block:
        ...
        #net = layers.max_pool2d(net, [3, 3, 3], stride=2, scope='pool1')
        net = slim.pool(net,[3, 3, 3], stride=2, scope='pool1', data_format = 'NDHWC', pooling_type='MAX')
      ...
      if global_pool:
        ...
        net = math_ops.reduce_mean(net, [1, 2, 3], name='pool5', keep_dims=True)
            ...
            with slim.arg_scope([slim.batch_norm], is_training=is_training):
                with slim.arg_scope([slim.conv2d, slim.conv2d_transpose, slim.pool], stride=1, padding='SAME'):
                    with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], activation_fn=tf.nn.relu):
                        ...
                        out = resnet_v2_50(inputs, num_classes=1, output_stride=8,  reuse=reuse)                       
                        print(out)

                        try:
                          net = out[-1]['resnet3D/resnet_v2_50/logits']
                        except:
                          net = out[-1]['resnet3D_1/resnet_v2_50/logits']
