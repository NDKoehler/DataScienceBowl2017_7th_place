...
def save_rank(filename, d):
    if os.path.exists(filename):
        old_d = load_json(filename)
        old_d.update(d)
        d = old_d
...
def train(*args, **kwargs):
    ...
    test_image_to_log = tf.placeholder(
        tf.uint8, [kwargs['batch_size'],kwargs['image_shape'][-3]*4, kwargs['image_shape'][-2]*4, 3])
    log_image_test = tf.summary.image("Test examples", test_image_to_log, max_outputs = kwargs['batch_size'])

    train_image_to_log = tf.placeholder(
        tf.uint8, [kwargs['batch_size'],kwargs['image_shape'][-3]*4, kwargs['image_shape'][-2]*4, 3])
    log_image_train = tf.summary.image("Train examples", train_image_to_log, max_outputs = kwargs['batch_size'])
    ...
    if my_metric_ops == None:
        ...
        for cnt, m in enumerate(test_metric_ops_list):
                va_summary_op_train, va_loss_placeholder = create_summary_op(
                    'va/' + test_metric_ops_list[0].name.split('_')[-len(test_metric_ops_list)+cnt])  # test_loss_op.name
                print(test_metric_ops_list[0].name.split('_')[-len(test_metric_ops_list)+cnt])
    ...
    for step in range(kwargs['max_steps']):
        ...
        if step % evaluation_step == 0:
            ...
            if my_metric_ops == None:
                ...
                for batch_losses in test_loss:
                    loss_val, predlabels = batch_losses
                    preds, labs = predlabels
                    loss_vals.append(np.reshape(loss_val, [loss_val.size,]))
                    predictions.append(np.reshape(preds, [preds.size,]))
                    labels.append(labs)

                predictions = np.concatenate(predictions, axis = 0)
                labels = np.concatenate(labels, axis = 0)

                predictions = np.reshape(predictions, [predictions.size,]).tolist()
                labels = np.reshape(labels, [labels.size,]).tolist()
                ...
                sorted_rank_true_positives = [x for (y,x) in sorted(zip(predictions,labels), key = lambda pair: pair[0], reverse = True)]

                filename = kwargs['output_dir'] + '/rank_log.json'
                d = {step : sorted_rank_true_positives}
                save_rank(filename, d)
