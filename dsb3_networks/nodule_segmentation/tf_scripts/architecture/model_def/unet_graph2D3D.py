...
def unet_graph2D3D(kwargs,
    ...
    with tf.variable_scope('3D-stage', reuse=reuse):
        net = slim.conv2d(inputs3D,    4  , [3, 3, 3], stride=1, scope='conv1',   reuse=reuse, data_format = 'NDHWC')
        net = slim.conv2d(net,         8  , [3, 3, 3], stride=1, scope='conv2',   reuse=reuse, data_format = 'NDHWC')
        net = slim.conv2d(net,         16 , [3, 3, 3], stride=1, scope='conv3',   reuse=reuse, data_format = 'NDHWC')

    print_shape(net)
    net = tf.transpose(net, [0,2,3,1,4]) # [batch_size, z, y, x, Ch] -> [batch_size, y, x, z, Ch]
    print_shape(net)
    shape = net.get_shape()
    inputs2D = tf.reshape(net, shape=[int(shape[0]),int(shape[1]),int(shape[2]),int(shape[3]*shape[4])])# [batch_size, z, y, x, 1] -> [batch_size, y, x, z]
    print_shape(net)
    ...
    # summaries or losses.
    end_points = {}
    with tf.variable_scope(scope, [inputs2D], reuse=reuse):
                ...
                with slim.arg_scope([slim.conv2d, slim.conv2d_transpose, slim.max_pool2d], stride=1, padding='SAME'):
                    with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], activation_fn=tf.nn.relu):
                        ...
                        with tf.variable_scope('in_240x240x32', reuse=reuse):
                            in240 = slim.conv2d(inputs2D,         kernel_num, [
                        ...
                        with tf.variable_scope('out_240x240x1', reuse=reuse):
                            ...
                            num_output_channels = kwargs[
                                'label_shape'][-1]  # inputs2D.get_shape()[-1]
